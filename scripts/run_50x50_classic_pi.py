...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run 50x50 classic full-action SMDP policy iteration.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=6,
        help="Local destination support radius. Set to -1 for full destination distribution.",
    )
...
def main() -> None:
    ...
    print("[stage] 50x50: classic full-action PI", flush=True)
    print(
        f"[params] actions_per_state={mdp.n_cells}, H={args.horizon}, gamma={args.discount}, "
        f"sweeps={args.evaluation_sweeps}, max_iterations={args.max_iterations}, "
        f"destination_radius={destination_label}",
        flush=True,
    )
    ...
    print("\nResult")
    print("method | actions_per_state | time_s | iterations | avg_reward | avg_empty")
    print("-------+-------------------+--------+------------+------------+----------")
    print(
        f"classic_pi | {mdp.n_cells} | {elapsed:.3f} | {result.stats.iterations} | "
